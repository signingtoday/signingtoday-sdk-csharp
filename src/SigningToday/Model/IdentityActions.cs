/* 
 * Signing Today API
 *
 * 
 *
 * The version of the OpenAPI document: 1.5.0
 * Contact: smartcloud@bit4id.com
 * Generated by: https://github.com/openapitools/openapi-generator.git
 */

using System;
using System.Linq;
using System.IO;
using System.Text;
using System.Text.RegularExpressions;
using System.Collections;
using System.Collections.Generic;
using System.Collections.ObjectModel;
using System.Runtime.Serialization;
using Newtonsoft.Json;
using Newtonsoft.Json.Converters;
using System.ComponentModel.DataAnnotations;
using OpenAPIDateConverter = SigningToday.Client.OpenAPIDateConverter;

namespace SigningToday.Model
{
    /// <summary>
    /// The object that identifies the urls to sign, to send a one time password or to update the certificate
    /// </summary>
    [DataContract]
    public partial class IdentityActions :  IEquatable<IdentityActions>, IValidatableObject
    {
        /// <summary>
        /// Initializes a new instance of the <see cref="IdentityActions" /> class.
        /// </summary>
        /// <param name="sendOtp">The url to get a one time password.</param>
        /// <param name="sign">The url to sign a document.</param>
        /// <param name="updateCertificate">The url to update the certificate.</param>
        public IdentityActions(string sendOtp = default(string), string sign = default(string), string updateCertificate = default(string))
        {
            this.SendOtp = sendOtp;
            this.Sign = sign;
            this.UpdateCertificate = updateCertificate;
        }
        
        /// <summary>
        /// The url to get a one time password
        /// </summary>
        /// <value>The url to get a one time password</value>
        [DataMember(Name="send_otp", EmitDefaultValue=false)]
        public string SendOtp { get; set; }

        /// <summary>
        /// The url to sign a document
        /// </summary>
        /// <value>The url to sign a document</value>
        [DataMember(Name="sign", EmitDefaultValue=false)]
        public string Sign { get; set; }

        /// <summary>
        /// The url to update the certificate
        /// </summary>
        /// <value>The url to update the certificate</value>
        [DataMember(Name="update_certificate", EmitDefaultValue=false)]
        public string UpdateCertificate { get; set; }

        /// <summary>
        /// Returns the string presentation of the object
        /// </summary>
        /// <returns>String presentation of the object</returns>
        public override string ToString()
        {
            var sb = new StringBuilder();
            sb.Append("class IdentityActions {\n");
            sb.Append("  SendOtp: ").Append(SendOtp).Append("\n");
            sb.Append("  Sign: ").Append(Sign).Append("\n");
            sb.Append("  UpdateCertificate: ").Append(UpdateCertificate).Append("\n");
            sb.Append("}\n");
            return sb.ToString();
        }
  
        /// <summary>
        /// Returns the JSON string presentation of the object
        /// </summary>
        /// <returns>JSON string presentation of the object</returns>
        public virtual string ToJson()
        {
            return JsonConvert.SerializeObject(this, Formatting.Indented);
        }

        /// <summary>
        /// Returns true if objects are equal
        /// </summary>
        /// <param name="input">Object to be compared</param>
        /// <returns>Boolean</returns>
        public override bool Equals(object input)
        {
            return this.Equals(input as IdentityActions);
        }

        /// <summary>
        /// Returns true if IdentityActions instances are equal
        /// </summary>
        /// <param name="input">Instance of IdentityActions to be compared</param>
        /// <returns>Boolean</returns>
        public bool Equals(IdentityActions input)
        {
            if (input == null)
                return false;

            return 
                (
                    this.SendOtp == input.SendOtp ||
                    (this.SendOtp != null &&
                    this.SendOtp.Equals(input.SendOtp))
                ) && 
                (
                    this.Sign == input.Sign ||
                    (this.Sign != null &&
                    this.Sign.Equals(input.Sign))
                ) && 
                (
                    this.UpdateCertificate == input.UpdateCertificate ||
                    (this.UpdateCertificate != null &&
                    this.UpdateCertificate.Equals(input.UpdateCertificate))
                );
        }

        /// <summary>
        /// Gets the hash code
        /// </summary>
        /// <returns>Hash code</returns>
        public override int GetHashCode()
        {
            unchecked // Overflow is fine, just wrap
            {
                int hashCode = 41;
                if (this.SendOtp != null)
                    hashCode = hashCode * 59 + this.SendOtp.GetHashCode();
                if (this.Sign != null)
                    hashCode = hashCode * 59 + this.Sign.GetHashCode();
                if (this.UpdateCertificate != null)
                    hashCode = hashCode * 59 + this.UpdateCertificate.GetHashCode();
                return hashCode;
            }
        }

        /// <summary>
        /// To validate all properties of the instance
        /// </summary>
        /// <param name="validationContext">Validation context</param>
        /// <returns>Validation Result</returns>
        IEnumerable<System.ComponentModel.DataAnnotations.ValidationResult> IValidatableObject.Validate(ValidationContext validationContext)
        {
            yield break;
        }
    }

}
