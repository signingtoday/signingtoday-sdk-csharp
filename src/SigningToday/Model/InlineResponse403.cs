...
namespace SigningToday.Model
{
    ...
    public partial class InlineResponse403 :  IEquatable<InlineResponse403>, IValidatableObject
    {
        ...
        /// </summary>
        /// <param name="detail">detail (default to &quot;Non sono state immesse le credenziali di autenticazione&quot;).</param>
        public InlineResponse403(string detail = "Non sono state immesse le credenziali di autenticazione")
        {
            // use default value if no "detail" provided
            if (detail == null)
            {
                this.Detail = "Non sono state immesse le credenziali di autenticazione";
            }
            else
            {
                this.Detail = detail;
