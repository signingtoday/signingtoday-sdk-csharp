/* 
 * Signing Today API
 *
 * 
 *
 * The version of the OpenAPI document: 1.5.0
 * Contact: smartcloud@bit4id.com
 * Generated by: https://github.com/openapitools/openapi-generator.git
 */

using System;
using System.Linq;
using System.IO;
using System.Text;
using System.Text.RegularExpressions;
using System.Collections;
using System.Collections.Generic;
using System.Collections.ObjectModel;
using System.Runtime.Serialization;
using Newtonsoft.Json;
using Newtonsoft.Json.Converters;
using System.ComponentModel.DataAnnotations;
using OpenAPIDateConverter = SigningToday.Client.OpenAPIDateConverter;

namespace SigningToday.Model
{
    /// <summary>
    /// UpdateOrganization
    /// </summary>
    [DataContract]
    public partial class UpdateOrganization :  IEquatable<UpdateOrganization>, IValidatableObject
    {
        /// <summary>
        /// Initializes a new instance of the <see cref="UpdateOrganization" /> class.
        /// </summary>
        /// <param name="defaultRao">This is the default *RAO* user of the Organization. A rao user is the one can associate identities to the other users .</param>
        /// <param name="dstDefaultMonths">This is the default deadline before the expiration of a digital signature transaction (default to 3).</param>
        /// <param name="logo">This is the url to the image is supposed to be used as logo of the Organization, for example the logo or the motto of the company is integrating SigningToday .</param>
        /// <param name="name">This is an arbitrary name is possible to associate to the Organization .</param>
        /// <param name="signatureAppearance">This is the url to the default signature appearance will be used for every member of the organization. In the scenario of a user that owns an identity with a signature_appearance will be uset the image associated to the identity rather than the default one .</param>
        public UpdateOrganization(string defaultRao = default(string), int dstDefaultMonths = 3, string logo = default(string), string name = default(string), string signatureAppearance = default(string))
        {
            this.DefaultRao = defaultRao;
            // use default value if no "dstDefaultMonths" provided
            if (dstDefaultMonths == null)
            {
                this.DstDefaultMonths = 3;
            }
            else
            {
                this.DstDefaultMonths = dstDefaultMonths;
            }
            this.Logo = logo;
            this.Name = name;
            this.SignatureAppearance = signatureAppearance;
        }
        
        /// <summary>
        /// This is the default *RAO* user of the Organization. A rao user is the one can associate identities to the other users 
        /// </summary>
        /// <value>This is the default *RAO* user of the Organization. A rao user is the one can associate identities to the other users </value>
        [DataMember(Name="default_rao", EmitDefaultValue=false)]
        public string DefaultRao { get; set; }

        /// <summary>
        /// This is the default deadline before the expiration of a digital signature transaction
        /// </summary>
        /// <value>This is the default deadline before the expiration of a digital signature transaction</value>
        [DataMember(Name="dst_default_months", EmitDefaultValue=false)]
        public int DstDefaultMonths { get; set; }

        /// <summary>
        /// This is the url to the image is supposed to be used as logo of the Organization, for example the logo or the motto of the company is integrating SigningToday 
        /// </summary>
        /// <value>This is the url to the image is supposed to be used as logo of the Organization, for example the logo or the motto of the company is integrating SigningToday </value>
        [DataMember(Name="logo", EmitDefaultValue=false)]
        public string Logo { get; set; }

        /// <summary>
        /// This is an arbitrary name is possible to associate to the Organization 
        /// </summary>
        /// <value>This is an arbitrary name is possible to associate to the Organization </value>
        [DataMember(Name="name", EmitDefaultValue=false)]
        public string Name { get; set; }

        /// <summary>
        /// This is the url to the default signature appearance will be used for every member of the organization. In the scenario of a user that owns an identity with a signature_appearance will be uset the image associated to the identity rather than the default one 
        /// </summary>
        /// <value>This is the url to the default signature appearance will be used for every member of the organization. In the scenario of a user that owns an identity with a signature_appearance will be uset the image associated to the identity rather than the default one </value>
        [DataMember(Name="signature_appearance", EmitDefaultValue=false)]
        public string SignatureAppearance { get; set; }

        /// <summary>
        /// Returns the string presentation of the object
        /// </summary>
        /// <returns>String presentation of the object</returns>
        public override string ToString()
        {
            var sb = new StringBuilder();
            sb.Append("class UpdateOrganization {\n");
            sb.Append("  DefaultRao: ").Append(DefaultRao).Append("\n");
            sb.Append("  DstDefaultMonths: ").Append(DstDefaultMonths).Append("\n");
            sb.Append("  Logo: ").Append(Logo).Append("\n");
            sb.Append("  Name: ").Append(Name).Append("\n");
            sb.Append("  SignatureAppearance: ").Append(SignatureAppearance).Append("\n");
            sb.Append("}\n");
            return sb.ToString();
        }
  
        /// <summary>
        /// Returns the JSON string presentation of the object
        /// </summary>
        /// <returns>JSON string presentation of the object</returns>
        public virtual string ToJson()
        {
            return JsonConvert.SerializeObject(this, Formatting.Indented);
        }

        /// <summary>
        /// Returns true if objects are equal
        /// </summary>
        /// <param name="input">Object to be compared</param>
        /// <returns>Boolean</returns>
        public override bool Equals(object input)
        {
            return this.Equals(input as UpdateOrganization);
        }

        /// <summary>
        /// Returns true if UpdateOrganization instances are equal
        /// </summary>
        /// <param name="input">Instance of UpdateOrganization to be compared</param>
        /// <returns>Boolean</returns>
        public bool Equals(UpdateOrganization input)
        {
            if (input == null)
                return false;

            return 
                (
                    this.DefaultRao == input.DefaultRao ||
                    (this.DefaultRao != null &&
                    this.DefaultRao.Equals(input.DefaultRao))
                ) && 
                (
                    this.DstDefaultMonths == input.DstDefaultMonths ||
                    (this.DstDefaultMonths != null &&
                    this.DstDefaultMonths.Equals(input.DstDefaultMonths))
                ) && 
                (
                    this.Logo == input.Logo ||
                    (this.Logo != null &&
                    this.Logo.Equals(input.Logo))
                ) && 
                (
                    this.Name == input.Name ||
                    (this.Name != null &&
                    this.Name.Equals(input.Name))
                ) && 
                (
                    this.SignatureAppearance == input.SignatureAppearance ||
                    (this.SignatureAppearance != null &&
                    this.SignatureAppearance.Equals(input.SignatureAppearance))
                );
        }

        /// <summary>
        /// Gets the hash code
        /// </summary>
        /// <returns>Hash code</returns>
        public override int GetHashCode()
        {
            unchecked // Overflow is fine, just wrap
            {
                int hashCode = 41;
                if (this.DefaultRao != null)
                    hashCode = hashCode * 59 + this.DefaultRao.GetHashCode();
                if (this.DstDefaultMonths != null)
                    hashCode = hashCode * 59 + this.DstDefaultMonths.GetHashCode();
                if (this.Logo != null)
                    hashCode = hashCode * 59 + this.Logo.GetHashCode();
                if (this.Name != null)
                    hashCode = hashCode * 59 + this.Name.GetHashCode();
                if (this.SignatureAppearance != null)
                    hashCode = hashCode * 59 + this.SignatureAppearance.GetHashCode();
                return hashCode;
            }
        }

        /// <summary>
        /// To validate all properties of the instance
        /// </summary>
        /// <param name="validationContext">Validation context</param>
        /// <returns>Validation Result</returns>
        IEnumerable<System.ComponentModel.DataAnnotations.ValidationResult> IValidatableObject.Validate(ValidationContext validationContext)
        {
            yield break;
        }
    }

}
